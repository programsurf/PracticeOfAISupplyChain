...
def encrypt_file_hybrid(public_key, input_file, output_file):
    ...
    # 1. 파일 읽기
    # TODO: 파일을 바이너리 읽기 모드("rb")로 열어서 내용을 읽으세요
    with open(input_file, "rb") as f:
        plaintext = None  # 여기를 수정하세요

    print(f"    File size: {len(plaintext)} bytes")

    # 2. AES 키 생성 (256-bit)
    # TODO: 32바이트 랜덤 AES 키를 생성하세요
    # 힌트: os.urandom(32)
    aes_key = None  # 여기를 수정하세요

    # TODO: 16바이트 랜덤 IV를 생성하세요
    # 힌트: os.urandom(16)
    iv = None  # 여기를 수정하세요
    ...
    # TODO: AES Cipher 객체를 생성하세요
    # 힌트: Cipher(algorithms.AES(aes_key), modes.CBC(iv), backend=default_backend())
    cipher = None  # 여기를 수정하세요
    ...
    # TODO: encryptor를 사용하여 암호화하세요
    # 힌트: encryptor.update(padded_plaintext) + encryptor.finalize()
    ciphertext = None  # 여기를 수정하세요

    print(f"    File encrypted with AES-256-CBC")

    # 4. RSA로 AES 키 암호화
    # TODO: RSA 공개키로 AES 키를 암호화하세요
    # 힌트: public_key.encrypt(aes_key, padding.OAEP(...))
    # OAEP 파라미터: mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None
    encrypted_key = None  # 여기를 수정하세요
...
def decrypt_file_hybrid(private_key, input_file, output_file):
    ...
    # 2. RSA로 AES 키 복호화
    # TODO: RSA 개인키로 AES 키를 복호화하세요
    # 힌트: private_key.decrypt(encrypted_key, padding.OAEP(...))
    # OAEP 파라미터: mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None
    aes_key = None  # 여기를 수정하세요
    ...
    # TODO: decryptor를 사용하여 복호화하세요
    # 힌트: decryptor.update(ciphertext) + decryptor.finalize()
    padded_plaintext = None  # 여기를 수정하세요
...
def main():
    ...
    # TODO: 파일 이름을 설정하세요
    # 원본 파일, 암호화된 파일, 복호화된 파일의 이름을 정의하세요
    original_file = None       # 예: "sample.txt"
    encrypted_file = None      # 예: "sample.txt.enc"
    decrypted_file = None      # 예: "sample_decrypted.txt"
